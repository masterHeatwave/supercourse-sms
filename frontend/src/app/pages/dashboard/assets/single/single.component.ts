...
export class SingleAssetComponent implements OnInit {
    ...
    this.inventoryService.getInventoryId(this.itemId).subscribe({
      ...
      error: (error) => {
        ...
        this.messageService.add({
          ...
          detail: error?.error?.message || this.translate.instant('assets.errors.load_failed')
        });
      }
    });
  }
  ...
  onEdit() {
    this.router.navigate(['/dashboard/assets/edit', this.itemId]);
  }
}
