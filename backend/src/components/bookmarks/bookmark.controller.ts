...
export class BookmarkController {
  ...
  getBookmarks = asyncHandler(async (req: Request, res: Response, _next: NextFunction) => {
    try {
      ...
      const statusCode = error instanceof ZodError || error instanceof mongoose.Error.ValidationError ? 400 : 500;
      return res.status(statusCode).json({
        statusCode,
        message: statusCode === 400 ? 'Validation error' : 'Internal server error',
